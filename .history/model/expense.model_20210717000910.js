...
class ExpenseModel{
    static async createExpense(title, amount, dateOfExpense, paymentMethod, categoryId, userId){
        try {
    ...
    static async editExpense(id,title, amount, dateOfExpense, paymentMethod, categoryId, userId){
        try {
            await new ExpenseSchema().expense.findOneAndUpdate({_id: id},{
                title,
                amount,
                dateOfExpense,
                paymentMethod,
    ...
    static async createExpenseCategory(name,userId){
        try {
            await new ExpenseCategorySchema().expenseCategory.create({name,userId});
            return true;
    ...
    static async getAllExpenseCategories(userId){
        try {
            const expenseCategories = await new ExpenseCategorySchema().expenseCategory.find({userId});
